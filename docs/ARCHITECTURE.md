...
```text
        ...
        +----------------------------+------------------------------------+----------------------+
                                     ...
                                     |   | - enforce per-user hold cap in Redis                                                        |
                                     |   | - publish payment work to SQS through a circuit breaker                                     |
                                     |   +----------------------------+-------------------------------------------+----------------------+
                                     |                                |                          |
                                     ...
                                     |   | - per-user hold counters with max 8 active    |        | - booking_seats                    |   | idempotencyKey                                   |
                                     ...
                                     |                                                                           +-------------------------------+
                                     |
                                     | API degraded mode when SQS publish fails for 60s; sync fallback is used for low-volume traffic
                                     v
                           +-----------------------------------------------+
                           | Synchronous payment fallback (degraded mode) |
                           | used only when breaker is open               |
                           +-----------------------------------------------+
```
...
- Redis Cluster -> CONCURRENCY.md: Redis absorbs hot contention with SETNX locks, TTL-based cache keys, and per-user hold caps.
